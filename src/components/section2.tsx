...
function Section2() {
      ...
      const observe = new IntersectionObserver(
        ([entry]) => {
          if (entry.isIntersecting) {
            ...
            opacity: [
              {to:0,duration:0},
              {to:1,duration:800,ease:'easeOutQuad'}
            ],
            translateX:[
              {to:200 ,duration:0},
              {to:0,duration:800,ease:'easeOutQuad'}
            ],
            delay: index * 150
  ...
  const div_cards_styles = "mx-auto bg-Warm_Black_plus w-56 p-4 h-auto mt-4 lg:mr-2 opacity-0"
  const p_cards_styles = "text-xs w-52 text-Grey pr-2 "

  return (
    <>
    <section ref={root} className="w-full h-auto relative text-Just_White bg-Warm_Black flex flex-col justify-center mx-auto " >
      ...
      <div className="mb-8 md:mx-auto md:flex-wrap md:flex md:justify-between" >
        <div className={`${div_cards_styles} card`} >
